...
Install for the current user. These commands track `main`, so they install the latest repository state. For reproducible installs after a release is tagged, replace `main` with a tag such as `v1.0.0`.
